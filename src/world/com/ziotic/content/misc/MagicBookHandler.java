...
public class MagicBookHandler implements ButtonHandler {
    ...
    public static boolean refreshModern(Player player) {
        ...
        Static.proto.sendVarp(player, 1376, (atr.get("showSkill", true) ? 0 : 2) << 9 | (atr.get("showTeleport", true) ? 0 : 2) << 11 | (atr.get("showMisc", true) ? 0 : 2) << 10 | (atr.get("showCombat", true) ? 0 : 2) << 8 | sortId);
        return true;
    }
    ...
    public static boolean refreshAncient(Player player) {
        Attributes atr = player.getAttributes();
        int sortId = atr.get("sortLevel", true) ? 0 : atr.get("sortCombat", false) ? 1 : 2;
        Static.proto.sendVarp(player, 1376, 1 << 9 | (atr.get("showCombat", true) ? 0 : 2) << 15 | (atr.get("showTeleport", true) ? 0 : 2) << 16 | sortId << 3);
        return true;
    }
    ...
    public static boolean refreshLunar(Player player) {
        Attributes atr = player.getAttributes();
        int sortId = atr.get("sortLevel", true) ? 0 : atr.get("sortCombat", false) ? 1 : 2;
        Static.proto.sendVarp(player, 1376, 2 << 9 | (atr.get("showCombat", true) ? 0 : 2) << 12 | (atr.get("showMisc", true) ? 0 : 2) << 13 | (atr.get("showTeleport", true) ? 0 : 2) << 14 | sortId << 6);
        return true;
    }
    ...
    public boolean handle(Player player, int interfaceId, int buttonId) {
        ...
        if (modern && buttonId == 15 || ancient && buttonId == 9 || lunar && buttonId == 11) {
            ...
            Static.proto.sendVarp(player, 439, atr.get("defensiveCast", false) ? (ancient ? 257 : (lunar ? 258 : 256)) : (ancient ? 1 : (lunar ? 2 : 0)));
            return true;
        }
        return false;
    }
