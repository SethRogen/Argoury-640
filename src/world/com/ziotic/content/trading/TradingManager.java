...
public class TradingManager {
    ...
    public static void setTradeStagesInitiation335(Player player, Player player2) {
    	int playerWealth = calculateTradeScreenWealth(player);
    	int otherWealth = calculateTradeScreenWealth(player.getTradingManager().otherPlayer);
        updateTradeStage(player, player2, TradeStage.INITIATED_335);
        updateTradeStage(player2, player, TradeStage.INITIATED_335);
        

        Static.proto.sendInterfaceVariable(player, 729, playerWealth);
        Static.proto.sendInterfaceVariable(player2, 697, otherWealth);
    }

    public static int calculateTradeScreenWealth(Player player) {
        int wealth = 0;
        for (PossesedItem item : player.getTradingManager().tradeScreenItems.array()) {
            if (item != null) {
                wealth += ItemPriceManager.getPrice(item.getId()) * item.getAmount();
            }
        }
        return wealth;
    }
        ...
        if (player2.isConnected() && player2.inGame() && player.isConnected() && player.inGame()) {
            switch (player.getTradingManager().stage) {
                ...
                case INITIATED_334:
                    ...
                    // Send lent items to update the interface
                    player.getTradingManager().sendLentItemInterface(player);
                    player2.getTradingManager().sendLentItemInterface(player2);
                    break;
            }
        }
    }
    ...
    /* 
    public static void sendLentItemInterface(Player player) {
    	Static.proto.sendInterfaceShowConfig(player, 334, 48, false);
	     Static.proto.sendString(player, 334, 50, "Lend Item");
	     
	    	Static.proto.sendInterfaceShowConfig(player, 334, 51, false);
		     Static.proto.sendString(player, 334, 52, "Lend Item");
    }*/
    
    public static void sendLentItemInterface(Player player) {
        Player other = player.getTradingManager().otherPlayer;
        if (other == null) return; // safety check
        // Player's lent item
        PossesedItem playerLent = player.getTradingManager().lentItem.size() > 0 ? player.getTradingManager().lentItem.get(0) : null;
        
     // Player A’s lent item → show “Lend” for them, “Borrow” for other
        if (playerLent != null) {
            ItemDefinition def = ItemDefinition.forId(playerLent.getId());
            String itemName = def.name;
            // Show for player (Lend)
            Static.proto.sendInterfaceShowConfig(player, 334, 48, false);
            Static.proto.sendInterfaceShowConfig(player, 334, 50, false);
            Static.proto.sendString(player, 334, 50, "Lend: <col=ffffff>" + itemName);
            // Show for other player (Borrow)
            Static.proto.sendInterfaceShowConfig(other, 334, 51, false);
            Static.proto.sendInterfaceShowConfig(other, 334, 52, false);
            Static.proto.sendString(other, 334, 52, "Borrow: <col=ffffff>" + itemName);
        } else {
            // Hide components if no item
            Static.proto.sendInterfaceShowConfig(player, 334, 48, true);
            Static.proto.sendInterfaceShowConfig(player, 334, 50, true);
            Static.proto.sendInterfaceShowConfig(other, 334, 51, true);
            Static.proto.sendInterfaceShowConfig(other, 334, 52, true);
        }
    }
    ...
    public static void refreshBoth(Player player, Player player2) {
        ...
        // For player
        int playerWealth = calculateTradeScreenWealth(player);
        int otherWealth = calculateTradeScreenWealth(player2);
        Static.proto.sendInterfaceVariable(player, 729, playerWealth);
        Static.proto.sendInterfaceVariable(player, 697, otherWealth);

        // For player2
        int player2Wealth = calculateTradeScreenWealth(player2);
        int playerWealthForOther = calculateTradeScreenWealth(player);
        Static.proto.sendInterfaceVariable(player2, 729, player2Wealth);
        Static.proto.sendInterfaceVariable(player2, 697, playerWealthForOther);
    }
