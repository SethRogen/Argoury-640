...
public abstract class PrayerManager implements ButtonHandler {
    ...
    public void load() throws IOException {
    	String dataFolder = Static.parseString(Constants.DATA_FOLDER);
        ...
        try {
             ...
             */
            curseDefinitions = (Map<Integer, AncientCurseDefinition>) Static.xml.readObject(Static.parseString(dataFolder + "/world/skills/prayer/ancientcurses.xml"));
            prayerDefinitions = (Map<Integer, PrayerDefinition>) Static.xml.readObject(Static.parseString(dataFolder + "/world/skills/prayer/prayers.xml"));
    ...
    /*
        ...
        Gson gson = new Gson();
        String dataFolder = Static.parseString(Constants.DATA_FOLDER);
        try (FileReader cursesReader = new FileReader(dataFolder + "/world/skills/prayer/ancientcurses.json");
             FileReader prayersReader = new FileReader(dataFolder + "/world/skills/prayer/prayers.json")) {
            Type curseType = new TypeToken<Map<Integer, AncientCurseDefinition>>() {}.getType();
            curseDefinitions = gson.fromJson(cursesReader, curseType);
            Type prayerType = new TypeToken<Map<Integer, PrayerDefinition>>() {}.getType();
            prayerDefinitions = gson.fromJson(prayersReader, prayerType);
            // Reset quick-select arrays
            Arrays.fill(quickSelectCurses, false);
            Arrays.fill(quickSelectPrayers, false);
            if (curseDefinitions != null && prayerDefinitions != null) {
                LOGGER.info("Loaded " + curseDefinitions.size() + " curses");
                LOGGER.info("Loaded " + prayerDefinitions.size() + " prayers");
            } else {
