...
import com.runescape.logic.item.ItemListener;
...
public class Shop extends RecurringEvent {
    ...
    public void sell(Player player, int index, int amount) {
        ...
        if (/*!item.getDefinition().isTradeable()*/ false) {
            Static.proto.sendMessage(player, "You cannot sell this item.");
            return;
        }
        if (acceptedItems != null && !acceptedItems.isEmpty() && !acceptedItems.contains(item.getId())) {
            Static.proto.sendMessage(player, "You cannot sell that item in this shop.");
            return;
        }
        ...
        for (PossesedItem shopItem : stock) {
            if (shopItem != null && shopItem.getId() == item.getId()) {
                ...
                int value = item.getDefinition().shopPrice * amount;
        ...
        Static.proto.sendMessage(player, "There is not enough space in the shop for this item.");
    }
