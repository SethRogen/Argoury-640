...
public class Firemaking implements ActionHandler, ItemOnItemHandler, GroundItemOptionHandler {
                        ...
                        Tick tick3 = new Tick(null, fireTime(player, medium)) {
                            ...
                            public boolean execute() {
                            	 for (Player p : Static.world.getPlayers()) {
                                     if (p.skillObject != null && p.skillObject instanceof CookType) {
                                         CookType cookType = (CookType) p.skillObject;
                                             cookType.burnedFire = true;
                                     }
                                 }
                                Static.world.getObjectManager().remove(src);
                                Static.world.getGroundItemManager().add(ASHES.getId(), 1, src, null, false);
                                return false;
                            }
                        };
    ...
    public static boolean isFire(int objectId) {
        switch(objectId) {
            case 2732:
            case 4266:
            case 4766:
