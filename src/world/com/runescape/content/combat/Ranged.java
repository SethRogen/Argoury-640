...
import com.runescape.content.combat.definitions.AmmunitionDefinition;
import com.runescape.content.combat.definitions.BowDefinition;
import com.runescape.content.combat.definitions.RangedDefinitions;
...
import com.runescape.logic.HPHandlerTick.HPAddition;
...
public class Ranged {
	...
	public static boolean shoot(Player player, Entity victim, ActionType type, boolean special) {
		if (canShoot(player, victim)) {
			...
			if (special) {
				if (attack != null) {
					boolean b = SpecialAttacks.handleSpecial(player, victim, player.getCombat(), attack, type, 0);
					if (!b) {
	...
	public static void load() throws IOException {
	    if (bows != null || projectiles != null) {
	        throw new IllegalStateException("Ranged definitions already loaded");
	    }
	    Gson gson = new Gson();
	    String path = Static.parseString(Constants.DATA_FOLDER + "/world/items/weapons/range.json");
	    try (FileReader reader = new FileReader(path)) {
	        RangedDefinitions definitions = gson.fromJson(reader, RangedDefinitions.class);
	        bows = definitions.bows.stream().collect(Collectors.toMap(b -> b.id, b -> b));
	        projectiles = definitions.ammunition.stream().collect(Collectors.toMap(a -> a.id, a -> a));
	        LOGGER.info("Loaded " + bows.size() + " bow(s)");
	        LOGGER.info("Loaded " + projectiles.size() + " ammunition(s)");
	    } catch (Exception e) {
	        LOGGER.error("Error loading ranged definitions", e);
	        throw new IOException("Failed to load Ranged JSON file", e);
	    }
	}
