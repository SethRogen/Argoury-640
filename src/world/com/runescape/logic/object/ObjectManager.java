...
public class ObjectManager {
    ...
    public void respawnTree(GameObject obj, int stumpId, int respawnTimeMillis) {
        Tile loc = obj.getLocation();
        if (loc == null) return;

        // Replace tree with stump
        add(stumpId, loc, obj.getType(), obj.getDirection());

        // Schedule respawn of the original tree
        Static.engine.scheduleRecurringEvent(new RecurringEvent(respawnTimeMillis, Event.ExecutorType.PARALLEL_LOGIC) {
            @Override
            public void run() {
                // Remove stump
                remove(loc);
                ...
                add(obj.getId(), loc, obj.getType(), obj.getDirection());

                stop(); // Stop the recurring event
            }
        });
    }
