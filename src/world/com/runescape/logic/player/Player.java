...
import com.runescape.content.skills.cooking.Cooking;
import com.runescape.content.skills.fletching.Fletching;
...
import com.runescape.logic.item.ItemSelection;
...
public class Player extends Entity implements PlayerType, Destroyable {
    ...
    public static final Tile NEW_PLAYER_LOCATION = Tile.locate(2677, 3299, 0);
    public static final Tile DEFAULT_RESPAWN_LOCATION = Tile.locate(2656, 3296, 0);
    ...
    public Tile spawnPoint() {
        if (noob && onLogin) {
            return NEW_PLAYER_LOCATION.randomize(11, 10);
        }
        return DEFAULT_RESPAWN_LOCATION.randomize(10, 20);
    }
    ...
    public transient SkillDialogue skillDialogue;

    public transient int maxSkillDialogueAmount, selectedSkillDialogueAmount;
    
    public transient ItemSelection<?> itemSelection;
    
    public void openSkillDialogue(SkillDialogue skillDialogue, int maxAmount, int... items) {
    	System.out.println("OpenSkillDia");
    	maxSkillDialogueAmount = maxAmount;
        selectedSkillDialogueAmount = maxAmount;
        Static.proto.sendInterface(this, 905, 752, 13, true); //ChatBox Interface ID
        Static.proto.sendInterface(this, 916, 905, 4, true);//Amount Interface ID
        Static.proto.sendString(this, 916, 1, skillDialogue.info); //Choose String
        Static.proto.sendInterfaceVariable(this, 754, skillDialogue.configId); //Options
        for(int i = 0; i < SkillDialogue.CHILDREN.length; i++) {
            int[] children = SkillDialogue.CHILDREN[i];
            if(i >= items.length) {
            	Static.proto.sendInterfaceVariable(this, children[0], -1);
                continue;
            }
            int itemId = items[i];
            Static.proto.sendInterfaceVariable(this, children[0], itemId);
            ItemDefinition def = ItemDefinition.forId(itemId);
            //String name = def != null ? def.name : "Unknown";
            Static.proto.sendSpecialString(this, children[1], def.name);
            this.skillDialogue = skillDialogue;
        } 
    }
    
    public void skillDialogueSelect(int slot) {
        if(skillDialogue == null) {
        	Static.proto.sendCloseChatboxInterface(this);
            return;
        }
        if(selectedSkillDialogueAmount <= 0) {
            removeSkillDialogue();
            Static.proto.sendCloseChatboxInterface(this);
            return;
        }
        switch(skillDialogue) {
            case COOKING:
                Cooking.cook(this, selectedSkillDialogueAmount);
                return;
            case CRAFTING:
               // Crafting.select(this, slot, selectedSkillDialogueAmount);
                return;
            case FLETCHING:
                Fletching.select(this, slot, selectedSkillDialogueAmount);
                return;
            case HERBLORE:
               // Herblore.select(this, selectedSkillDialogueAmount);
                return;
        }
    }
    
    public void removeSkillDialogue() {
        skillDialogue = null;
        skillObject = null;
    }
    
    public void openItemSelection(ItemSelection<?> selection) {
            ...
            case POST_UPDATE:
                try {
                    ...
                    if (sendMapRegionUpdate) {
                    	...
                    	//Static.proto.sendDynamicRegion(this);
                    }
                } catch (Throwable e) {
    ...
    public final void playJingle(int id, int delay) {
        Static.proto.sendRSJingle(this, id, delay);
    }
}
