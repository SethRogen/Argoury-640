...
public class Levels {
    ...
    public void addXP(int skillId, double amount) {
        ...
        if (oldLevel < newLevel) { // The level changed.
    		...
    		player.getAttributes().set("leveledUp", skillId);
    		player.getAttributes().set("leveledUp["+skillId+"]", Boolean.TRUE);
    		player.doGraphics(199, 0, 100);
    		Static.proto.sendMessage(player, "You've just advanced a " + Levels.SKILL_NAME[skillId] + " level! You have reached level " + newLevel + ".");
    		if (newLevel == 99 || newLevel == 120) {
    			player.doGraphics(1633, 0 , 100);
    			player.doGraphics(1635, 0 , 100);
    			Static.proto.sendMessage(player, "<col=800000>Well done! You've achieved the highest possible level in this skill!</col>");
    		}
    		Static.proto.sendString(player, 740, 0 , "Congratulations, you have just advanced a " + Levels.SKILL_NAME[skillId] + " level!");
    		Static.proto.sendString(player, 740, 1, "You have now reached level " + newLevel + ".");
    		Static.proto.sendConfig(player, 1179, SKILL_ICON[skillId]); //TODO FLASHING SKILL ICONS
