...
import com.runescape.adapter.protocol.cache.format.ItemDefinitionAdapter;
...
public class ItemDefinition {
    ...
    public static void loadEquipmentIds() {
        try {
            String path = Static.parseString(Constants.DATA_FOLDER +"/world/items/equipids.json");
            File file = new File(path);
            if (file.exists()) {
                try (FileReader reader = new FileReader(file)) {
                    Type type = new TypeToken<HashMap<Integer, Integer>>() {}.getType();
                    equipmentIds = gson.fromJson(reader, type);
                    if (equipmentIds == null)
                        equipmentIds = new HashMap<>();
                }
            } else {
                equipmentIds = new HashMap<>();
                ...
                try (FileWriter writer = new FileWriter(file)) {
                    gson.toJson(equipmentIds, writer);
                }
            }
