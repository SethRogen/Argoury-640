...
public class ItemContainer {
    ...
    public void replace(int itemId, int replaceId) {
        int slot = getSlot(itemId);
        if (slot == -1) {
            return;
        }

        PossesedItem oldItem = items[slot];
        if (oldItem == null) {
            return;
        }
        ...
        if (replaceId == -1) {
            ...
            items[slot] = new PossesedItem(replaceId, amount);
        }

        event(ItemListener.ItemEventType.CHANGE, slot);
    }
    
    public int getSlot(int itemId) {
        for (int i = 0; i < items.length; i++) {
            PossesedItem item = items[i];
