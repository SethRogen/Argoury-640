...
public class LinkServer implements Application {
    ...
    public void main(String[] args) throws Throwable {
        ...
        worldList = loadWorldListFromSQL();
    ...
    private Map<Integer, WorldEntry> loadWorldListFromSQL() throws Exception {
        Map<Integer, WorldEntry> worlds = new HashMap<>();
        SQLSession session = null;
        try {
            session = sqlPool.acquire();
            Statement st = session.createStatement();
            ResultSet rs = st.executeQuery("SELECT * FROM world_list");
            while (rs.next()) {
                WorldEntry w = new WorldEntry();
                w.id = rs.getInt("id");
                w.activity = rs.getString("activity");
                w.countryId = rs.getInt("country_id");
                w.countryName = rs.getString("country_name");
                w.members = rs.getBoolean("members");
                w.highlight = rs.getBoolean("highlight");
                w.lootShare = rs.getBoolean("loot_share");
                w.quickChat = rs.getBoolean("quick_chat");
                w.highRisk = rs.getBoolean("high_risk");
                w.skillReq = rs.getBoolean("skill_req");
                worlds.put(w.id, w);
            }
            rs.close();
            st.close();
