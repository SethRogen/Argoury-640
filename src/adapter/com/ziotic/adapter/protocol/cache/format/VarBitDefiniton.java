...
public class VarBitDefiniton {
	
	 private static final ConcurrentHashMap<Integer, VarBitDefiniton> varpbitDefs = new ConcurrentHashMap<Integer, VarBitDefiniton>();
		...
		public VarBitDefiniton(Player player) {
			this.player = player;
			values = new int[Static.rs2Cache.getIndex(22).getLength() * 1024];
		}
	
    public static final VarBitDefiniton getClientVarpBitDefinitions(Player player, int id) {
    VarBitDefiniton script = varpbitDefs.get(id);
	if (script != null)
	    return script;
	byte[] data = Static.rs2Cache.getIndex(22).getArchivedFile(id >>> 10, id & 0x3ff);
	System.out.println("Load ConfigByFile [id] = " + id);
	System.out.println();
	System.out.println("There are currently: " + Static.rs2Cache.getIndex(22).getLength() * 0x3ff + " bitConfigs.");
	script = new VarBitDefiniton(player);
	script.id = id;
	if (data != null)
	...
	private void sendVar(Player player, int id, int value, boolean force) {
	...
	private static void setVarBit(Player player, int id, int value, int flag) {
		...
		VarBitDefiniton defs = getClientVarpBitDefinitions(player, id);
	...
	private static void sendClientVarp(Player player, int id) {
		Static.proto.sendVarp(player, id, values[id]);
	}
