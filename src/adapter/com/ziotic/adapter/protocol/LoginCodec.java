...
import com.ziotic.network.StandardFrameDecoder;
        ...
        if (buffer.remaining() >= 3) {
            ...
            do {
                if (buffer.remaining() >= size) {
                    ...
                    IoBuffer buffer2;
                    switch (opcode) {
                    	case 28: //Account Creation Connection
                    			System.out.println("Account Connection Step 2");
                            	int affId, lang, game, uuid,  additionalInfo, age;
                            	String email;
                            	boolean subscribe;
                            	long userFlow;
                        		size = buffer.getShort(); //PacketSize
                        		if (size > buffer.remaining()) {
                                return true;
                        		}
                        		clientRevision = buffer.getShort();
                        		if (clientRevision != Static.clientConf.getClientVersion()) {
                        		}
