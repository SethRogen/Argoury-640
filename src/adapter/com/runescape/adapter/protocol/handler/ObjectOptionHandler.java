...
import com.runescape.logic.object.ObjectOptionRegistry;
                ...
                Runnable r2 = new Runnable() {
                    ...
                    public void run() {
                        if (!Static.ahs.handleObjectOption(player, obj, index)) {
                        	ObjectHandler handler = ObjectOptionRegistry.getHandler(obj.getId());
                        	if (handler != null) {
                        	    boolean handled = false;
                        	    switch (index) {
                        	        case 1:
                        	            handled = handler.handleOption1(player, obj);
                        	            break;
                        	        case 2:
                        	            handled = handler.handleOption2(player, obj);
                        	            break;
                        	        case 3:
                        	            handled = handler.handleOption3(player, obj);
                        	            break;
                        	    }
                        	    if (!handled) {
