...
import com.runescape.content.clanchat.Clan.Rank;
import com.runescape.engine.login.LoginResponse;
import com.runescape.link.WorldEntry;
...
import com.runescape.logic.map.DynamicRegion;
...
public final class ProtocolAdapter implements Protocol {
        ...
        for (String friendObj : friends) {
            ...
            if (friend != null) {
                ...
                if (friend.inLobby()) {
                    ...
                    sb.append(Constants.GAME_NAME + " ").append(' ');
                }
    ...
    @Override
	public Protocol sendDynamicRegion(Player player) {
		FrameBuilder fb = new FrameBuilder(105, Frame.FrameType.VAR_SHORT, 256);
		int middleChunkX = player.getLocation().getPartX();
		int middleChunkY = player.getLocation().getPartY();
		fb.writeLEShort(middleChunkX). //ChunkX
		writeByteC(2). //Not sure
		writeByteC(1). //Force Refresh
		writeLEShort(middleChunkY). //ChunkY
		writeByteA(0); //Map Size 0?
		int sceneLength = Constants.REGION_SIZE[0] >> 4;
		...
		for (int plane = 0; plane < 4; plane++) {
			for (int realChunkX = (middleChunkX - sceneLength); realChunkX <= ((middleChunkX + sceneLength)); realChunkX++) {
				...
				y: for (int realChunkY = (middleChunkY - sceneLength); realChunkY <= ((middleChunkY + sceneLength)); realChunkY++) {
					...
					Region region = Region.forRegionCoordinates(regionX, regionY);
					int newChunkX;
					int newChunkY;
					int newPlane;
					int rotation;
					...
					if (newChunkX == 0 || newChunkY == 0)
						...
						fb.writeBits(1, 1);
						fb.writeBits(26, (rotation << 1) | (newPlane << 24) | (newChunkX << 14) | (newChunkY << 3));
		...
		for (int index = 0; index < newRegionIdsCount; index++) {
			int[] xteas = Static.mapXTEA.getKey(regionIds[index]);
			if (xteas == null)
				xteas = new int[4];
			for (int keyIndex = 0; keyIndex < 4; keyIndex++)
				fb.writeInt(xteas[keyIndex]);
		}
		player.getSession().write(fb.toFrame());
		return this;
	}
    ...
    public Protocol sendMapRegion(Player player) {
        ...
        fb.
        writeLEShortA(player.getLocation().getPartX()).
        writeByte(0).
        writeByteA(1).
        writeShortA(player.getLocation().getPartY());
    ...
    /* (non-Javadoc)
      * @see Protocol#sendInterface(com.runescape.network.GameInterface)
      */
    ...
    //Matrix Uses this and is called (sendIComponentSettings)
    @Override 
    public Protocol sendAccessMask(Player player, int interfaceId, int componentId, int fromSlot, int toSlot, int settingsHash) {
        FrameBuilder fb = new FrameBuilder(41, 12);
        fb
        .writeLEShortA(toSlot)
        .writeLEShortA(fromSlot)
        .writeLEInt(interfaceId << 16 | componentId)
        .writeInt2(settingsHash);
        player.getSession().write(fb.toFrame());
        return this;
    }
    ...
    @Override //Matrix (sendRunScript)
    public Protocol sendRunScript(Player player, int scriptId, Object... params) {
    	FrameBuilder fb = new FrameBuilder(50, Frame.FrameType.VAR_SHORT, 512);
			...
			for (int count = parameterTypes.length() - 1; count >= 0; count--) {
				if (parameterTypes.charAt(count) == 's')
					//stream.writeString((String) params[index++]);
				 	fb.writeString((String) params[index++]);
				else
					//stream.writeInt((Integer) params[index++]);
					fb.writeInt((Integer) params[index++]);
			}
		}
		player.getSession().write(fb.toFrame());
        return this;
    ...
    @Override //Matrix (sendGlobalConfig)
    ...
    @Override //Matrix is called (sendHideIComponent)
    public Protocol sendInterfaceShowConfig(Player player, int interfaceId, int childId, boolean hidden) {
        ...
        .writeByteA(hidden ? 1 : 0)
        .writeInt1(interfaceId << 16 | childId);
    ...
    @Override
	public Protocol sendRSJingle(Player player, int id, int delay) {
    	FrameBuilder fb = new FrameBuilder(114, 8);
		fb.writeShort(id);
		fb.writeByte(1);
		fb.writeShort(delay);
		fb.writeByte(255);
